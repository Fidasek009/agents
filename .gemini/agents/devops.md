...
- Use `run_shell_command` to run build, deployment, lint, policy, and validation commands.
- Use `grep_search` and `glob` to map pipeline files, manifests, scripts, and environment configs.
- Use the `context7` MCP for up-to-date docs on Kubernetes, Terraform, GitHub Actions, Helm, and cloud tooling.
- Use the `google_web_search` tool for incident signatures, CVEs, and vendor advisories.
