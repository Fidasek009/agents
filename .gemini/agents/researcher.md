...
- Use `web_fetch` and `google_web_search` as primary tools for internet research and latest updates.
- Use the `context7` MCP for official library documentation, API changes, and version-specific references.
- Use `read_file`, `glob`, and `grep_search` to align external findings with current repository context.
- Use `TodoWrite` to structure multi-step investigations and reporting.
- Use `ask_user` when scope, jurisdiction, timeline, or confidence requirements are unclear.
