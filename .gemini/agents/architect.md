...
- Use `read_file`, `glob`, and `grep_search` to map existing architecture, constraints, and conventions.
- Use `web_fetch` and `google_web_search` to validate current best practices and framework capabilities.
- Use `ask_user` when key constraints are missing (budget, scale, compliance, timeline).
