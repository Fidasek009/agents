...
- Use `read_file`, `glob`, and `grep_search` to discover existing test patterns, frameworks, and critical code paths.
- Use `run_shell_command` to run test commands, coverage reports, and static checks.
- Use `TodoWrite` for multi-step QA plans and staged test implementation.
- Use `web_fetch` and the `context7` MCP for framework-specific testing best practices.
- Use `ask_user` when acceptance criteria or expected behavior is unclear.
